...
namespace FYProject
{
    ...
    public class ProcedurePreload : ProcedureBase
    {
        ...
        {
            m_LoadedFlag[string.Format("Config.{0}", configName)] = false;
            GameEntry.Config.LoadConfig(configName, this);
        }

        private void LoadDataTable(string dataTableName)
        {
            m_LoadedFlag[string.Format("DataTable.{0}", dataTableName)] = false;
            GameEntry.DataTable.LoadDataTable(dataTableName, this);
        }

        private void LoadDictionary(string dictionaryName)
        {
            m_LoadedFlag[string.Format("Dictionary.{0}", dictionaryName)] = false;
